...
{$HPPEMIT '#pragma link "TANavigation"'}

interface

uses
  Classes, Controls, Graphics, StdCtrls, TAChartUtils, TAGraph, Types;

type

  { TChartNavScrollBar }

  TChartNavScrollBar = class(TScrollBar)
  private
...
begin
  ...
  case Kind of
    sbHorizontal: begin
      ...
      le.b.X := le.b.X + (d - le.a.X);
      le.a.X := d;
    end;
    sbVertical: begin
      d := WeightedAverage(fe.b.Y, fe.a.Y, Position / w);
      le.b.Y := le.b.Y + (d - le.a.Y);
      le.a.Y := d;
    end;
  end;
...
begin
  if (Chart <> nil) and FIsDragging then begin
    ...
    le := Chart.LogicalExtent;
    le.a := le.a + (p - FPrevPoint);
    le.b := le.b + (p - FPrevPoint);
    Chart.LogicalExtent := le;
    FPrevPoint := p;
  end;
