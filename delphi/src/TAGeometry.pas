...
type
  ...
  public
    FC: TDoublePoint;
    FR: TDoublePoint;
    procedure InitBoundingBox(AX1, AY1, AX2, AY2: Integer);
  public
...
function DoublePoint(AX, AY: Double): TDoublePoint; overload; inline;
function DoublePoint(const AP: TPoint): TDoublePoint; overload; inline;
function DoubleRect(AX1, AY1, AX2, AY2: Double): TDoubleRect; inline;
procedure ExpandRect(var ARect: TDoubleRect; const APoint: TDoublePoint); overload; inline;
procedure ExpandRect(var ARect: TRect; const APoint: TPoint); overload; inline;
procedure ExpandRect(
  var ARect: TRect; const ACenter: TPoint; ARadius: Integer;
  AAngle1, AAngle2: Double); overload; inline;
function IsPointOnLine(const AP, A1, A2: TPoint): Boolean;
function IsPointInPolygon(
  const AP: TPoint; const APolygon: array of TPoint): Boolean;
function IsPointInRect(const AP, A1, A2: TPoint): Boolean; overload; inline;
function IsPointInRect(const AP: TPoint; const AR: TRect): Boolean; overload; inline;
...
// Delphi's TPoint lives in the RTL and cannot be given operators, so the
// TPoint arithmetic that Lazarus expresses with operator overloads is provided
// as functions here.  TPoint + TPoint and TPoint - TPoint are omitted because
// System.Types already declares them.
function AddPoint(const A: TPoint; const B: TSize): TPoint; overload; inline;
function NegPoint(const A: TPoint): TPoint; inline;
function DivPoint(const A: TPoint; ADivisor: Integer): TPoint; inline;
function MulPoint(const A: TPoint; AMultiplier: Integer): TPoint; overload; inline;
function MulPoint(const A, B: TPoint): TPoint; overload; inline;
function MulPoint(const A: TPoint; const B: TSize): TPoint; overload; inline;
function PointToSize(const APoint: TPoint): TSize; inline;
function SizeToPoint(const ASize: TSize): TPoint; inline;

// The LCL declares these in its Types unit; Delphi's System.Types does not.
function Size(cx, cy: Integer): TSize; overload; inline;
function Size(const ARect: TRect): TSize; overload; inline;
function DivSize(const ASize: TSize; ADivisor: Integer): TSize; inline;
...
var
  ...
  s1, s2: TValueSign;
begin
  if Length(APolygon) = 0 then begin Result := false; exit; end;
  p1 := APolygon[High(APolygon)];
  for i := 0 to High(APolygon) do begin
    p2 := APolygon[i];
    if IsPointOnLine(AP, p1, p2) then begin Result := true; exit; end;
    p1 := p2;
  end;
  count := 0;
  p1 := APolygon[High(APolygon)];
  for i := 0 to High(APolygon) do begin
    ...
    s2 := Sign(p2.Y - AP.Y);
    case s1 * s2 of
      -1: count := count + Ord(PointLineSide(AP, p1, p2) = Sign(p1.Y - p2.Y));
      0: if s1 + s2 = 1 then begin
        if s1 = 0 then
          count := count + Ord(p1.X >= AP.X)
        else
          count := count + Ord(p2.X >= AP.X)
      end;
    end;
    p1 := p2;
  end;
...
var
  ...
  p1, p2: TPoint;
begin
  if (Length(AP1) = 0) or (Length(AP2) = 0) then begin Result := false; exit; end;
...
function LineIntersectsRect(
  ...
  procedure AdjustX(var AP: TDoublePoint; ANewX: Double);
  var
    dx: Double;
  begin
    dx := AB.X - AA.X;
    if not IsInfinite(dx) and not IsInfinite(AP.Y) then
      AP.Y := AP.Y + ((AB.Y - AA.Y) / dx * (ANewX - AP.X));
    AP.X := ANewX;
  end;

  procedure AdjustY(var AP: TDoublePoint; ANewY: Double);
  var
    dy: Double;
  begin
    dy := AB.Y - AA.Y;
    if not IsInfinite(dy) and not IsInfinite(AP.X) then
      AP.X := AP.X + ((AB.X - AA.X) / dy * (ANewY - AP.Y));
    AP.Y := ANewY;
  end;
...
begin
  case CASE_OF_TWO[AA.X < ARect.a.X, AB.X < ARect.a.X] of
    ...
    cotBoth: begin Result := false; exit; end;
  end;
...
var
  ...
  function Next(AIndex, ADir: Integer): Integer;
  begin
  ...
  function CrossProductSignByIndex(AIndex: Integer): Integer;
  begin
  ...
  begin
    ...
    if det = 0 then begin Result := AB; exit; end;
    with (AFrom - AA) do // Workaround for issue #17005.
      t := (Y * line.X - X * line.Y) / det;
    if t <= 0 then begin Result := AB; exit; end;
...
begin
  ...
  then
    begin Result := AShape; exit; end;
  ni := NearestSide;
  if ni < 0 then begin Result := AShape; exit; end;
...
begin
  if A = B then
    ...
    // Projection of P onto line A-B, seen from A.  The intermediate product
    // is kept in 64 bits; Lazarus relies on its TPoint * Integer operator here
    // and would truncate `dot` to 32 bits first.
    Q.X := (Int64(v.X) * dot) div lv;
    Q.Y := (Int64(v.Y) * dot) div lv;
...
begin
  with ARect do begin
    if Min(Right - Left, Bottom - Top) < 2 * ARadius then begin Result := nil; exit; end;
...
function AddPoint(const A: TPoint; const B: TSize): TPoint;
begin
  Result.X := A.X + B.cx;
  Result.Y := A.Y + B.cy;
end;

function NegPoint(const A: TPoint): TPoint;
begin
  Result.X := - A.X;
  Result.Y := - A.Y;
end;

function DivPoint(const A: TPoint; ADivisor: Integer): TPoint;
begin
  Result.X := A.X div ADivisor;
  Result.Y := A.Y div ADivisor;
end;

function MulPoint(const A: TPoint; AMultiplier: Integer): TPoint;
begin
  Result.X := A.X * AMultiplier;
  Result.Y := A.Y * AMultiplier;
end;

function MulPoint(const A, B: TPoint): TPoint;
begin
  Result.X := A.X * B.X;
  Result.Y := A.Y * B.Y;
end;

function MulPoint(const A: TPoint; const B: TSize): TPoint;
begin
  Result.X := A.X * B.cx;
  Result.Y := A.Y * B.cy;
end;

function PointToSize(const APoint: TPoint): TSize;
begin
  Result.cx := APoint.X;
  Result.cy := APoint.Y;
end;

function SizeToPoint(const ASize: TSize): TPoint;
begin
  Result.X := ASize.cx;
  Result.Y := ASize.cy;
end;
...
function DivSize(const ASize: TSize; ADivisor: Integer): TSize;
begin
  Result.cx := ASize.cx div ADivisor;
  Result.cy := ASize.cy div ADivisor;
end;
...
procedure TEllipse.InitBoundingBox(AX1, AY1, AX2, AY2: Integer);
begin
  FC.X := (AX1 + AX2) / 2;
  FC.Y := (AY1 + AY2) / 2;
  FR.X := Abs(AX1 - AX2) / 2;
