...
uses
  Classes, Contnrs, TAFPTypes, Graphics, SysUtils, TAChartUtils, TADrawUtils,
  TATypes, Types;
...
implementation
uses
  Math, TADrawerCanvas, TAGeometry;
...
begin
  ...
  FBackgroundBrush := InitHelper<TChartLegendBrush>;
  FFont := InitHelper<TFont>;
  FFrame := InitHelper<TChartPen>;
  FGroupFont := InitHelper<TFont>;
  FSymbolFrame := InitHelper<TChartPen>;
end;
    ...
    with AData do begin
      ...
      for i := 0 to FItems.Count - 1 do begin
        ...
        case ItemFillOrder of
          lfoColRow: begin x := i div FRowCount; y := i mod FRowCount; end;
          lfoRowCol: begin y := i div FColCount; x := i mod FColCount; end;
        end;
        if isRTL then
          r := Bounds(
...
begin
  ...
  for li in AItems do begin
    ...
    if li.HasSymbol then
      p.X := p.X + (ADrawer.Scale(SYMBOL_TEXT_SPACING + SymbolWidth));
...
begin
  ...
  sidebar.X := sidebar.X + legendSize.X;

  sidebar.Y := 2 * margX;
  with AClipRect do
    legendSize.Y := EnsureRange(legendSize.Y, 0, Bottom - Top - sidebar.Y);
  sidebar.Y := sidebar.Y + legendSize.Y;
  ...
  if UseSidebar then
    case Alignment of
      laTopLeft, laCenterLeft, laBottomLeft:
        AClipRect.Left := AClipRect.Left + sidebar.X;
      laTopRight, laCenterRight, laBottomRight:
        AClipRect.Right := AClipRect.Right - sidebar.X;
      laTopCenter:
        AClipRect.Top := AClipRect.Top + (legendSize.Y + 2 * margY);
      laBottomCenter:
        AClipRect.Bottom := AClipRect.Bottom - (legendSize.Y + 2 * margY);
    end;
