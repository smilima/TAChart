...
uses
  Classes, Math, Types, SysUtils, TALCLCompat;

const
  CHART_COMPONENT_IDE_PAGE = 'TAChart';
  PERCENT = 0.01;
...
type
  ...
  // Delphi requires operators to be members of the record they act on, so the
  // global operator overloads the Lazarus version declares in TAGeometry live
  // here instead.  Their implementations are at the bottom of this unit.
  TDoublePoint = record
    X, Y: Double;
    class operator Add(const A, B: TDoublePoint): TDoublePoint;
    class operator Subtract(const A, B: TDoublePoint): TDoublePoint;
    class operator Multiply(const A, B: TDoublePoint): TDoublePoint; overload;
    class operator Multiply(const A: TDoublePoint; B: Double): TDoublePoint; overload;
    class operator Divide(const A, B: TDoublePoint): TDoublePoint;
    class operator Equal(const A, B: TDoublePoint): Boolean;
    class operator NotEqual(const A, B: TDoublePoint): Boolean;
    class operator LessThanOrEqual(const A, B: TDoublePoint): Boolean;
  end;

  TDoubleRect = record
  public
    class operator Equal(const A, B: TDoubleRect): Boolean;
    class operator NotEqual(const A, B: TDoubleRect): Boolean;
  case Integer of
    0: (
      a, b: TDoublePoint;
    );
  ...
  TTypedFPListEnumerator<T: class> = class(TChartListEnumerator)
  {$ELSE}
  TTypedFPListEnumerator = class(TChartListEnumerator)
  {$ENDIF}
  ...
  TIndexedComponentList = class(TList)
  public
  ...
  // Lazarus resource strings are ordinary string variables, so it points one
  // at them with a PString.  Delphi keeps resource strings in the resource
  // table and @rsFoo yields a PResStringRec, which has to be resolved with
  // LoadResString - dereferencing it as a string corrupts the heap.
  PStr = PResStringRec;
...
begin
  // Bitwise comparison, so that two NaNs with the same payload compare
  // equal and +0 does not compare equal to -0.
  Result := PInt64(@A1)^ = PInt64(@A2)^;
end;
...
procedure THistory<TElem>.Add(const AItem: TElem);
begin
...
procedure THistory<TElem>.DeleteOld(ACount: Integer);
begin
  FCount := FCount - ACount;
  Move(FData[ACount], FData[0], NativeInt(FCount) * SizeOf(FData[0]));
end;

function THistory<TElem>.GetCapacity: Cardinal;
begin
  Result := Length(FData);
end;

function THistory<TElem>.GetItem(AIndex: Integer): TElem;
begin
  if AIndex < 0 then
    AIndex := AIndex + (Integer(FCount));
  Result := FData[AIndex];
end;

function THistory<TElem>.Pop: TElem;
begin
  Result := GetItem(-1);
  FCount := FCount - 1;
end;

procedure THistory<TElem>.SetCapacity(AValue: Cardinal);
begin
...
function TTypedFPListEnumerator<T>.GetCurrent: T;
begin
...
begin
  if AnsiStartsStr(AOld, Name) then
    try
      ...
      AFailed := AFailed + (IfThen(AFailed = '', '', ', ') + Name);
    end;
end;

{ TIntervalList }

procedure TIntervalList.AddPoint(APoint: Double);
begin
...
procedure TPublishedIntegerSet.Init;
begin
  FAllSet := true;
end;
...
begin
  ...
  if FCaptionPtr <> nil then FCaption := LoadResString(ACaptionPtr);
end;
...
begin
  ...
  if item.FCaptionPtr <> nil then
    Result := LoadResString(item.FCaptionPtr) else
...
class operator TDoublePoint.Add(const A, B: TDoublePoint): TDoublePoint;
begin
  Result.X := A.X + B.X;
  Result.Y := A.Y + B.Y;
end;

class operator TDoublePoint.Subtract(const A, B: TDoublePoint): TDoublePoint;
begin
  Result.X := A.X - B.X;
  Result.Y := A.Y - B.Y;
end;

class operator TDoublePoint.Multiply(const A, B: TDoublePoint): TDoublePoint;
begin
  Result.X := A.X * B.X;
  Result.Y := A.Y * B.Y;
end;

class operator TDoublePoint.Multiply(const A: TDoublePoint; B: Double): TDoublePoint;
begin
  Result.X := A.X * B;
  Result.Y := A.Y * B;
end;

class operator TDoublePoint.Divide(const A, B: TDoublePoint): TDoublePoint;
begin
  Result.X := A.X / B.X;
  Result.Y := A.Y / B.Y;
end;

class operator TDoublePoint.Equal(const A, B: TDoublePoint): Boolean;
begin
  Result := (A.X = B.X) and (A.Y = B.Y);
end;

class operator TDoublePoint.NotEqual(const A, B: TDoublePoint): Boolean;
begin
  Result := (A.X <> B.X) or (A.Y <> B.Y);
end;

class operator TDoublePoint.LessThanOrEqual(const A, B: TDoublePoint): Boolean;
begin
  Result := (A.X <= B.X) and (A.Y <= B.Y);
end;
...
class operator TDoubleRect.Equal(const A, B: TDoubleRect): Boolean;
begin
  Result := (A.a = B.a) and (A.b = B.b);
end;

class operator TDoubleRect.NotEqual(const A, B: TDoubleRect): Boolean;
begin
  Result := (A.a <> B.a) or (A.b <> B.b);
end;

initialization

  DrawData := TDrawDataRegistry.Create;
  DefSeparatorSettings := FormatSettings;
