...
uses
  Classes, Graphics, Controls, TAFPTypes, TAChartUtils, TADrawUtils, Types;
...
type
  TTACustomChart = class(TCustomControl)
  public
    // TControl.Color is protected in the VCL but public in the LCL; the chart
    // elements read it to pick their default background.
    property Color;
  public
  ...
  {$NODEFINE TCustomChart} // C++ must use TTACustomChart; TCustomChart collides with TeeChart
  TCustomChart = TTACustomChart;
  ...
  strict protected
    FOwner: TTACustomChart;
    FVisible: Boolean;
    // Lazarus passes a metaclass here, relying on TFPCanvasHelper's virtual
    // constructor.  TGraphicsObject has no virtual constructor, so the helper
    // class is supplied as a generic type parameter instead: Delphi resolves
    // T.Create to the concrete constructor when the method is specialised.
    function InitHelper<T: TGraphicsObject, constructor>: T;
    procedure SetVisible(AValue: Boolean);
    procedure StyleChanged(Sender: TObject); virtual;
  protected
    function GetOwner: TPersistent; override;
  public
    constructor Create(AOwner: TTACustomChart);
  public
    procedure Assign(ASource: TPersistent); override;

    procedure SetOwner(AOwner: TTACustomChart);
  ...
  TChartExtentHistory = THistory<TDoubleRect>;
  ...
  public
    ...
    function GetValue(AIndex: Integer): TChartDistance;
...
constructor TChartElement.Create(AOwner: TTACustomChart);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
function TChartElement.InitHelper<T>: T;
begin
  Result := T.Create;
  Result.OnChange := StyleChanged;
end;

procedure TChartElement.SetOwner(AOwner: TTACustomChart);
begin
  FOwner := AOwner;
end;
...
constructor TSeriesPointer.Create(AOwner: TTACustomChart);
begin
  inherited Create(AOwner);

  FBrush := InitHelper<TBrush>;
  FPen := InitHelper<TChartPen>;
...
constructor TChartMargins.Create(AOwner: TTACustomChart);
begin
  inherited Create(AOwner);
...
begin
  ARect.TopLeft := ARect.TopLeft - (Point(ADrawer.Scale(Left), ADrawer.Scale(Top)));
  ARect.BottomRight := ARect.BottomRight + (Point(ADrawer.Scale(Right), ADrawer.Scale(Bottom)));
end;

function TChartMargins.GetValue(AIndex: Integer): TChartDistance;
begin
...
constructor TChartArrow.Create(AOwner: TTACustomChart);
begin
  inherited Create(AOwner);
...
begin
  ...
  pt1 := AEndPos + MulPoint(RotatePointX(diag, AAngle - da), sgn);
  pt2 := AEndPos + MulPoint(RotatePointX(diag, AAngle + da), sgn);
  if BaseLength > 0 then begin
    ptBase := AEndPos + MulPoint(RotatePointX(-ADrawer.Scale(BaseLength), AAngle), sgn);
    ADrawer.SetBrushParams(bsSolid, APen.Color);
...
constructor TChartShadow.Create(AOwner: TTACustomChart);
begin
  inherited Create(AOwner);
  FColor := clBlack;
