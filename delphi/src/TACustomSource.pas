...
type
  ...
  published
    ...
    property Tolerance: Cardinal read FTolerance write SetTolerance default 1;
  end;
...
type
  ...
  public
    ...
    procedure SetX(AIndex: Integer; AValue: Double); overload;
    procedure SetX(AValue: Double); overload;
    procedure SetY(AIndex: Integer; AValue: Double); overload;
    procedure SetY(AValue: Double); overload;
  ...
  TValuesInRangeParams = record
    ...
    FUseY: Boolean;
    // Pixel range of [FMin, FMax], filled with direct GraphToImage calls.
    // ToImage interpolates these so interval math does not depend on of-object
    // method pointers (broken on Win64x when stored in this record).
    FImageLo, FImageHi: Integer;
    FMinLengthPx, FMaxLengthPx: Integer;
    ...
    function ToImage(AX: Double): Integer;
  end;
...
begin
  for i := AStart to AEnd - 1 do
    if AValues[i].FValue > AValues[i + 1].FValue then begin Result := false; exit; end;
  Result := true;
end;
...
begin
  with FIntervals do
    Result := not (
      (aipUseMinLength in Options) and (AStep < FMinLengthPx) or
      (aipUseMaxLength in Options) and (AStep > FMaxLengthPx));
end;

procedure TValuesInRangeParams.RoundToImage(var AValue: Double);

  function A2I(AX: Double): Integer;
  begin
    Result := ToImage(AX);
  end;
...
begin
  ...
  if span = 0 then
    ...
    t := (AX - FMin) / span;
    Result := Round(EnsureRange(
      FImageLo + t * (FImageHi - FImageLo), -MaxInt, MaxInt));
  end;
end;
...
begin
  FOwner := AOwner;
  // Do not use SetPropDefaults for these: Delphi RTTI default for set
  // properties is unreliable, and a wrong Options bit (aipGraphCoords)
  // wrecks auto-interval calculation.
  FCount := 5;
  FMaxLength := 50;
  FMinLength := 10;
  FOptions := DEF_INTERVAL_OPTIONS;
...
function TBasicChartSource.IsUpdating: Boolean;
begin
...
begin
  with AItem do begin
    ...
    for i := 0 to Min(High(FSum.YList), High(YList)) do
      FSum.YList[i] := FSum.YList[i] - YList[i];
  end;
end;
...
var
  i: Integer;
begin
  if FExtentIsValid then begin Result := FExtent; exit; end;
  FExtent := EmptyExtent;
  ...
  try
    ...
    for i := AStart to AEnd do begin
      ...
      while true do begin
        ...
        next := (NativeUInt(p) - NativeUInt(@AValues[0])) div SizeOf(p^);
        {$HINTS ON}
...
procedure TCustomChartSource.ValuesInRange(
  ...
  procedure Put(
    out ADest: TChartValueText; AValue: Double; AIndex: Integer);
  var
    nx, ny: Double;
  begin
...
var
  ...
  begin
    Result := False;
    if not (aipUseMinLength in AParams.FIntervals.Options) then
      Exit;
    imagePos := AParams.ToImage(AValue);
    Result := Abs(imagePos - prevImagePos) < AParams.FMinLengthPx;
    if not Result then
...
var
  i: Integer;
begin
  if FValuesTotalIsValid then begin Result := FValuesTotal; exit; end;
  FValuesTotal := 0;
  for i := 0 to Count - 1 do
    with Item[i]^ do
      FValuesTotal := FValuesTotal + (NumberOr(Y));
...
begin
  for i := 0 to Count - 1 do
    with Item[i]^ do
      if Y = Extent.b.Y then begin Result := GetX(AIndex); exit; end;
  Result := 0.0;
end;
...
begin
  for i := 0 to Count - 1 do
    with Item[i]^ do
      if Y = Extent.a.Y then begin Result := GetX(AIndex); exit; end;
  Result := 0.0;
end;
