...
{$HPPEMIT '#pragma link "TADataTools"'}

interface

uses
  Classes, TAChartUtils, TAGraph, TATools, TATextElements, TATypes, Types;
...
implementation
uses
  Math, SysUtils, TAChartStrConsts, TAChartAxis, TACustomSeries, TAGeometry;
...
begin
  Result := 0;
  case AUnits of
    cuPercent: begin Result := 0; exit; end; // Not implemented.
    cuAxis: begin
...
begin
  ...
  if Marks.Visible then begin
    ...
    p1 := DivPoint(p1 + p2, 2);
    a := a + (IfThen((dpdoLabelAbove in Options) xor flip, -Pi / 2, Pi / 2));
...
function TDataPointDistanceTool.SameTransformations(
  ...
  function CheckAxis(AAxisIndex1, AAxisIndex2: Integer): Boolean;
  begin
    Result :=
