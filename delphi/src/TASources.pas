...
begin
  ...
  with FSource[Index]^ do begin
    ...
    if FSource.YCount > 0 then
      Result := Result + (Format('|%g', [Y], fs));
    for i := 0 to High(YList) do
      Result := Result + (Format('|%g', [YList[i]], fs));
    Result := Result + (Format('|%s|%s', [IntToColorHex(Color), Text]));
  end;
end;
...
var
  rest: TDoubleDynArray;
  i: Integer;
begin
  if Length(AY) = 0 then
    raise EYListEmptyError.Create('AddXYList: Y List is empty');
  Result := Add(AX, AY[0], ALabel, AColor);
  if Length(AY) > 1 then begin
    // Lazarus writes AY[1..High(AY)] here; Delphi has neither array slices
    // nor Copy() on an open array parameter.
    SetLength(rest, Length(AY) - 1);
    for i := 1 to High(AY) do
      rest[i - 1] := AY[i];
    SetYList(Result, rest);
  end;
end;
...
begin
  ...
  if FValuesTotalIsValid then
    FValuesTotal := FValuesTotal + (NumberOr(AValue) - NumberOr(oldY));
  UpdateExtent;
  Notify;
end;
...
begin
  ...
  if FValuesTotalIsValid then
    FValuesTotal := FValuesTotal + (NumberOr(AY));
  Notify;
end;
...
procedure TCalculatedChartSource.CalcDerivative(AIndex: Integer);
  ...
  begin
    for j := 0 to ACount - 1 do begin
      ...
      FItem.Y := FItem.Y + (prevItem^.Y * ADir * ACoeffs[j]);
      for i := 0 to High(FItem.YList) do
        FItem.YList[i] := FItem.YList[i] + (prevItem^.YList[i] * ADir * ACoeffs[j]);
    end;
  end;
...
begin
  if Origin = nil then begin Result := nil; exit; end;
  Result := @FItem;
