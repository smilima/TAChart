...
{$HPPEMIT '#pragma link "TASeries"'}

interface

uses
  Classes, Graphics, Types, TAChartUtils, TADrawUtils, TACustomSeries,
  TALegend, TARadialSeries, TATypes;
...
type
  ...
  TBeforeDrawBarEvent = procedure (
    ASender: TTABarSeries; ACanvas: TCanvas; const ARect: TRect;
    APointIndex, AStackIndex: Integer; var ADoDefaultDrawing: Boolean
  ) of object;

  { TTABarSeries }

  TTABarSeries = class(TBasicPointSeries)
  private
    FBarBrush: TBrush;
  ...
  {$NODEFINE TBarSeries} // C++ must use TTABarSeries
  TBarSeries = TTABarSeries;

  { TTAPieSeries }

  TTAPieSeries = class(TCustomPieSeries)
  public
    property Radius;
  published
    property EdgePen;
  ...
  {$NODEFINE TPieSeries} // C++ must use TTAPieSeries
  TPieSeries = TTAPieSeries;

  TConnectType = (ctLine, ctStepXY, ctStepYX);

  { TTAAreaSeries }

  TTAAreaSeries = class(TBasicPointSeries)
  private
    FAreaBrush: TBrush;
  ...
  {$NODEFINE TAreaSeries} // C++ must use TTAAreaSeries
  TAreaSeries = TTAAreaSeries;
  ...
  TTALineSeries = class(TBasicPointSeries)
  private
    FLinePen: TPen;
    FLineType: TLineType;
  ...
  {$NODEFINE TLineSeries} // C++ must use TTALineSeries
  TLineSeries = TTALineSeries;

  // 'TSerie' alias is for compatibility with older versions of TAChart.
  // Use TTALineSeries instead.
  TSerie = TTALineSeries deprecated;
...
uses
  TAFPTypes, Math, SysUtils, TAChartStrConsts, TADrawerCanvas, TAGeometry,
  TAGraph, TAMath, TAStyles;

{ TTALineSeries }

procedure TTALineSeries.AfterDrawPointer(
...
procedure TTALineSeries.Assign(ASource: TPersistent);
begin
  if ASource is TTALineSeries then
    with TTALineSeries(ASource) do begin
...
constructor TTALineSeries.Create(AOwner: TComponent);
begin
...
destructor TTALineSeries.Destroy;
begin
  FreeAndNil(FLinePen);
  inherited;
end;

procedure TTALineSeries.Draw(ADrawer: IChartDrawer);
var
  ext: TDoubleRect;
  i: Integer;
begin
...
procedure TTALineSeries.DrawSingleLineInStack(
...
var
  ...
  function PolylineIsTooLong: Boolean;
...
procedure TTALineSeries.GetLegendItems(AItems: TChartLegendItems);
var
  lp: TPen;
  p: TSeriesPointer;
  i: Integer;
...
function TTALineSeries.GetSeriesColor: TColor;
begin
  Result := FLinePen.Color;
end;

function TTALineSeries.GetShowLines: Boolean;
begin
  Result := FLineType <> ltNone;
end;

procedure TTALineSeries.SetLinePen(AValue: TPen);
begin
  FLinePen.Assign(AValue);
end;

procedure TTALineSeries.SetLineType(AValue: TLineType);
begin
...
procedure TTALineSeries.SetSeriesColor(AValue: TColor);
begin
  FLinePen.Color := AValue;
end;

procedure TTALineSeries.SetShowLines(Value: Boolean);
begin
...
procedure TTALineSeries.SetShowPoints(AValue: Boolean);
begin
...
var
  ...
  begin
    // The buffer is $AARRGGBB while TColor is $00BBGGRR, so red and blue have
    // to be swapped.  Lazarus writes the TColor value unchanged here, which
    // renders these points with red and blue exchanged.
    PCardinal(rawImage.Data)[APoint.Y * r.Right + APoint.X] :=
      TColorToPixel32(ColorDef(AColor, SeriesColor));
    cnt := cnt + 1;
  end;
...
begin
  ...
  r := BoundsSize(0, 0, PointToSize(ParentChart.ClipRect.BottomRight - topLeft));
...
procedure TTABarSeries.Assign(ASource: TPersistent);
begin
  if ASource is TTABarSeries then
    with TTABarSeries(ASource) do begin
...
procedure TTABarSeries.BarOffsetWidth(
  AX: Double; AIndex: Integer; out AOffset, AWidth: Double);
var
  r: Double;
begin
...
constructor TTABarSeries.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TTABarSeries.Destroy;
begin
  FreeAndNil(FBarPen);
...
procedure TTABarSeries.Draw(ADrawer: IChartDrawer);
var
...
function TTABarSeries.Extent: TDoubleRect;
var
  x, ofs, w: Double;
  i: Integer;
begin
...
function TTABarSeries.GetBarWidth(AIndex: Integer): Integer;
var
  ofs, w: Double;
  f: TGraphToImageFunc;
begin
...
function TTABarSeries.GetLabelDataPoint(AIndex: Integer): TDoublePoint;
var
  ofs, w: Double;
begin
  Result := inherited GetLabelDataPoint(AIndex);
  BarOffsetWidth(TDoublePointBoolArr(Result)[IsRotated], AIndex, ofs, w);
  TDoublePointBoolArr(Result)[IsRotated] :=
    TDoublePointBoolArr(Result)[IsRotated] + ofs;
end;

procedure TTABarSeries.GetLegendItems(AItems: TChartLegendItems);
begin
  GetLegendItemsRect(AItems, BarBrush);
end;

function TTABarSeries.GetNearestPoint(const AParams: TNearestPointParams;
  out AResults: TNearestPointResults): Boolean;
var
  pointIndex: Integer;
...
function TTABarSeries.GetSeriesColor: TColor;
begin
  Result := FBarBrush.Color;
end;

function TTABarSeries.GetZeroLevel: Double;
begin
  Result := ZeroLevel;
end;

function TTABarSeries.IsZeroLevelStored: boolean;
begin
  Result := ZeroLevel <> 0.0;
end;

procedure TTABarSeries.SetBarBrush(Value: TBrush);
begin
  FBarBrush.Assign(Value);
end;

procedure TTABarSeries.SetBarOffsetPercent(AValue: Integer);
begin
...
procedure TTABarSeries.SetBarPen(Value:TPen);
begin
  FBarPen.Assign(Value);
end;

procedure TTABarSeries.SetBarWidthPercent(Value: Integer);
begin
...
procedure TTABarSeries.SetBarWidthStyle(AValue: TBarWidthStyle);
begin
...
procedure TTABarSeries.SetOnBeforeDrawBar(AValue: TBeforeDrawBarEvent);
begin
...
procedure TTABarSeries.SetSeriesColor(AValue: TColor);
begin
  FBarBrush.Color := AValue;
end;

procedure TTABarSeries.SetZeroLevel(AValue: Double);
begin
...
function TTABarSeries.ToolTargetDistance(const AParams: TNearestPointParams;
...
procedure TTAAreaSeries.Assign(ASource: TPersistent);
begin
  if ASource is TTAAreaSeries then
    with TTAAreaSeries(ASource) do begin
...
constructor TTAAreaSeries.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TTAAreaSeries.Destroy;
begin
...
procedure TTAAreaSeries.Draw(ADrawer: IChartDrawer);
var
  pts: TPointArray;
  numPts: Integer;
  scaled_depth: Integer;
...
function TTAAreaSeries.Extent: TDoubleRect;
begin
  Result := inherited Extent;
...
procedure TTAAreaSeries.GetLegendItems(AItems: TChartLegendItems);
begin
  GetLegendItemsRect(AItems, AreaBrush);
end;

function TTAAreaSeries.GetSeriesColor: TColor;
begin
  Result := FAreaBrush.Color;
end;

function TTAAreaSeries.GetZeroLevel: Double;
begin
  Result := IfThen(UseZeroLevel, ZeroLevel, 0.0);
end;

function TTAAreaSeries.IsZeroLevelStored: boolean;
begin
  Result := ZeroLevel <> 0.0;
end;

procedure TTAAreaSeries.SetAreaBrush(AValue: TBrush);
begin
  FAreaBrush.Assign(AValue);
  UpdateParentChart;
end;

procedure TTAAreaSeries.SetAreaContourPen(AValue: TPen);
begin
  FAreaContourPen.Assign(AValue);
  UpdateParentChart;
end;

procedure TTAAreaSeries.SetAreaLinesPen(AValue: TPen);
begin
  FAreaLinesPen.Assign(AValue);
  UpdateParentChart;
end;

procedure TTAAreaSeries.SetConnectType(AValue: TConnectType);
begin
...
procedure TTAAreaSeries.SetSeriesColor(AValue: TColor);
begin
  FAreaBrush.Color := AValue;
end;

procedure TTAAreaSeries.SetUseZeroLevel(AValue: Boolean);
begin
...
procedure TTAAreaSeries.SetZeroLevel(AValue: Double);
begin
  if FZeroLevel = AValue then exit;
  FZeroLevel := AValue;
  UpdateParentChart;
...
initialization
  RegisterSeriesClass(TTALineSeries, @rsLineSeries);
  RegisterSeriesClass(TTAAreaSeries, @rsAreaSeries);
  RegisterSeriesClass(TTABarSeries, @rsBarSeries);
  RegisterSeriesClass(TTAPieSeries, @rsPieSeries);
