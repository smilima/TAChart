...
// Lazarus declares `operator := (AEq: IFitEquationText): String`, letting an
// equation builder be assigned straight to a string.  Delphi cannot attach
// operators to an interface, so callers must finish the chain with .Get:
//   Label1.Caption := Series.EquationText.X('x').NumFormat('%.3f').Get;
...
begin
  ps := '';
  if Length(FParams) = 0 then begin Result := ''; exit; end;
  ...
  if FEquation in [fePolynomial, feLinear] then
    ...
    case FEquation of
      feExp:
        Result := Result + (Format(' * exp(' + GetNumFormat(1) +' * %s)', [FParams[1], FX]));
      fePower:
        Result := Result + (Format(' * %s^' + GetNumFormat(1), [FX, FParams[1]]));
    end;
end;
