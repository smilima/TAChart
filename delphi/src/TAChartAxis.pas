...
uses
  Classes, SysUtils, Types, TAChartAxisUtils, TAChartUtils, TACustomSource,
  TADrawUtils, TATransformations, TATypes;
...
//    procedure VisitSource(ASource: TCustomChartSource; var AData);
  ...
  public
    ...
    procedure PrepareHelper(
      ...
      AXToImage, AYToImage: TAChartUtils.TGraphToImageFunc;
  ...
  strict private
    ...
    FZOrder: TList;
    procedure InitAndSort(AList: TList; ACompare: TListSortCompare);
  protected
    function GetOwner: TPersistent; override;
    procedure Update(AItem: TCollectionItem); override;
  public
...
uses
  Math, TAChartStrConsts, TAGeometry, TAMath;
...
begin
  if AAlignment in [calLeft, calTop] then
    ADelta := -ADelta;
  a[AAlignment] := a[AAlignment] + ADelta;
end;
...
begin
  if not Visible then begin Result := nil; exit; end;
...
begin
  Result := LoadResString(SIDE_NAME[Alignment]);
...
function TChartAxis.IsVertical: Boolean;
begin
...
begin
  Result := Default(TValuesInRangeParams);
  Result.FMin := AMin;
  Result.FMax := AMax;
  ...
  Result.FMinStep := 0;
  // Use the clip rectangle, not GraphToImage: of-object / interface conversion
  // of X values is unreliable on Win64x and produced thousands of tick labels.
  FHelper.GetClipRange(Result.FImageLo, Result.FImageHi);
  Result.FMinLengthPx := Intervals.MinLength;
  Result.FMaxLengthPx := Intervals.MaxLength;
  if Result.FMinLengthPx < 2 then
    Result.FMinLengthPx := 10;
  if Result.FMaxLengthPx < 2 then
    Result.FMaxLengthPx := 50;
end;
...
begin
  if IsDefaultPosition then begin Result := r[Alignment]; exit; end;
  case PositionUnits of
    ...
    cuAxis, cuGraph:
      if IsVertical then
        Result := FHelper.FXToImage(Position)
      else
        Result := FHelper.FYToImage(Position);
    cuPixel:
      Result :=
        r[Alignment] +
...
begin
  ...
  FHelper.FXToImage := AXToImage;
  FHelper.FYToImage := AYToImage;
...
function TChartAxisList.Add: TChartAxis;
begin
...
procedure TChartAxisList.InitAndSort(
  AList: TList; ACompare: TListSortCompare);
var
  a: TChartAxis;
begin
  AList.Clear;
...
begin
  ...
  for i := 0 to High(FGroups) do begin
    ...
    if g^.FCount = 0 then Continue;
    // Axises of the same group should have the same Alignment, Position and ZPosition.
    if Assigned(axis) and axis.IsDefaultPosition then
      Result[axis.Alignment] := Result[axis.Alignment] + Max(0,
  ...
  for i := 0 to FGroupOrder.Count - 1 do
    with TChartAxis(FGroupOrder[i]) do begin
      if (Group = 0) or (Group <> prevGroup) then begin
        ...
        FGroups[groupCount - 1].FCount := FGroups[groupCount - 1].FCount + 1;
...
procedure TAxisCoeffHelper.Init(
  AAxis: TChartAxis; AImageLo, AImageHi, AMarginLo, AMarginHi: Integer;
  AMin, AMax: PDouble);
begin
  FAxis := AAxis;
...
begin
  if (FMax^ = FMin^) or (Sign(FHi - FLo) <> ASign) then begin Result := 1.0; exit; end;
