...
{$HPPEMIT '#pragma link "TAFuncSeries"'}

interface

uses
  Classes, Graphics, TANumLib, Types, TAChartUtils, TACustomFuncSeries,
  TACustomSeries, TACustomSource, TADrawUtils, TAFitUtils, TALegend, TATypes;
...
type
  ...
  strict protected
    ...
    procedure Transform(AX, AY: Double; out ANewX, ANewY: Double);
  protected
...
implementation
uses
  TAFPTypes, Math, StrUtils, SysUtils, TAChartStrConsts, TAGeometry, TAGraph,
  TAMath;
...
begin
  Result :=
    ...
    X(AXText).Y(AYText).NumFormat(ANumFormat).Params(AParams).Get;
end;
...
begin
  AResults.FIndex := -1;
  if not Assigned(OnCalculate) then begin Result := false; exit; end;
...
begin
  ok := 0;
  if IsFewPoints then begin Result := SafeNaN; exit; end;
...
begin
  ...
  if IsFewPoints then begin Result := true; exit; end;
  ok := 0;
...
begin
  for s in FSplines do begin
    ...
    if not s.FIntervals.Intersect(x, x, hint) then
      begin Result := s.Calculate(AX); exit; end;
  end;
  Result := SafeNaN;
end;
...
var
  r: Integer;
  minv, maxv: ArbFloat;
  extY: TDoubleInterval;
  s: TSpline;
begin
  extY.FStart := Infinity;
  extY.FEnd := NegInfinity;
  r := 0;
...
var
  i: Integer;
begin
  if IsInfinite(AX) then begin Result := AX; exit; end;
  Result := SafeNaN;
...
var
  ...
  function IsValidPoint(AX, AY: Double): Boolean;
  begin
...
begin
  Result := EquationText.NumFormat(ANumFormat).X(AXText).Y(AYText).Get;
end;
...
begin
  ...
  then begin
    ExecFit;
    if State <> fpsValid then begin Result := false; exit; end;
    de := PrepareIntervals;
    try
      with TDrawFuncHelper.Create(Self, de, Calculate, Step) do
        try
...
procedure TFitSeries.Transform(AX, AY: Double; out ANewX, ANewY: Double);
begin
...
var
  ...
  v1, v2: Double;
begin
  if (ColorSource = nil) or (ColorSource.Count = 0) then begin Result := clTAColor; exit; end;
  ...
  try
    ...
    while pt.Y <= r.Bottom do begin
      ...
      while pt.X <= r.Right do begin
        ...
        gp := GraphToAxis(ParentChart.ImageToGraph(DivPoint(pt + next, 2)));
...
var
  ...
  var
    idx: Integer;
  begin
    if AText <> '' then begin Result := AText; exit; end;
    if ColorSource.Count = 1 then begin Result := ''; exit; end;
