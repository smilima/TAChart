...
uses
  Classes, TAFPTypes, Types, TAChartUtils;
...
type
  ...
  public
    ...
    procedure Done;
    function Pos(AX, AY: Integer): TChartTextOut; overload;
    function Pos(const APos: TPoint): TChartTextOut; overload;
    function Text(const AText: String): TChartTextOut; overload;
    function Text(AText: TStrings): TChartTextOut; overload;
  ...
  IChartDrawer = interface
    ...
    procedure DrawLineDepth(AX1, AY1, AX2, AY2, ADepth: Integer); overload;
    procedure DrawLineDepth(const AP1, AP2: TPoint; ADepth: Integer); overload;
    ...
    function TextExtent(const AText: String): TPoint; overload;
    function TextExtent(AText: TStrings): TPoint; overload;
  ...
  public
    ...
    procedure DrawLineDepth(const AP1, AP2: TPoint; ADepth: Integer); overload;
    function GetRightToLeft: Boolean;
    procedure LineTo(AX, AY: Integer); overload; virtual; abstract;
    procedure LineTo(const AP: TPoint); overload;
    procedure MoveTo(AX, AY: Integer); overload; virtual; abstract;
    procedure MoveTo(const AP: TPoint); overload;
    procedure Polygon(
...
begin
  with Result do begin
    ...
    green := green + (green shr 8);
    blue := (AChartColor and $FF0000) shr 8;
    blue := blue + (blue shr 8);
    alpha := alphaOpaque;
  end;
end;
...
begin
  ...
  for i := 0 to FText2.Count - 1 do begin
    ...
    p := FPos;
    case FAlignment of
      taCenter: p := p + RotatePointX((FWidth - lineExtent.X) div 2, a);
      taRightJustify: p := p + RotatePointX(FWidth - lineExtent.X, a);
    end;
    FSimpleTextOut.SimpleTextOut(p.X, p.Y, FText2[i]);
    FPos := FPos + (RotatePoint(Point(0, lineExtent.Y + LINE_INTERVAL), a));
  end;
end;

procedure TChartTextOut.DoTextOutString;
begin
  if System.Pos(sLineBreak, FText1) = 0 then begin
...
var
  sl: TStrings;
begin
  if Pos(sLineBreak, AText) = 0 then
    begin Result := SimpleTextExtent(AText); exit; end;
