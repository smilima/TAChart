...
uses
  Classes, Graphics, TAChartUtils, TACustomSource, TADrawUtils,
  TAIntervalSources, TAStyles, TATypes, TATextElements, Types;
...
type
  ...
  TCustomChartAxisTitle =
    TGenericChartMarks<TChartAxisBrush, TChartPen, TChartAxisFramePen>;
  {$ENDIF}
  ...
  public
    constructor Create(ACollection: TCollection; AChart: TCustomChart); reintroduce; overload;
    destructor Destroy; override;
  public
  ...
  public
    ...
    // Direct chart converters — not ICoordTransformer. Delphi cannot take a
    // stable method pointer from an interface, and assigning the virtual
    // GraphToImage below to TValuesInRangeParams broke X-axis tick spacing.
    FXToImage: TAChartUtils.TGraphToImageFunc;
    FYToImage: TAChartUtils.TGraphToImageFunc;
    FValueMax: Double;
    FValueMin: Double;
    FMaxForMarks: Double;
    FMinForMarks: Double;
    ...
    // Non-virtual wrapper so TValuesInRangeParams can store a stable method
    // pointer. Assigning GraphToImage itself is a virtual/abstract of-object
    // pointer and produced thousands of X-axis ticks on Win64.
    function MapToImage(AGraph: Double): Integer;
    // Delphi cannot take a method pointer from an interface reference, so the
    // drawer's Scale is republished as an ordinary method of this helper.
    function ScaleDistance(ADistance: Integer): Integer;
  end;
...
uses
  Math, SysUtils, TAGeometry, TAMath;
...
function TAxisDrawHelper.ScaleDistance(ADistance: Integer): Integer;
begin
  Result := FDrawer.Scale(ADistance);
end;

function TAxisDrawHelper.MapToImage(AGraph: Double): Integer;
begin
  Result := GraphToImage(AGraph);
end;
...
begin
  ...
  Result.FXToImage := FXToImage;
  Result.FYToImage := FYToImage;
...
begin
  ALabelCenter := ALabelCenter + FZOffset;
  ...
  if FAxis.Arrow.Visible then begin
    ...
    if FAxis.IsFlipped then begin
      ...
      if not arrowFlipped then
        arrowBase := arrowBase - (RotatePointX(-FDrawer.Scale(FAxis.Arrow.Length), AAngle));
    end else begin
...
begin
  if FAxis.IsFlipped then begin
    ...
    if FAxis.Arrow.Visible then
      p1.X := p1.X - (FDrawer.Scale(FAxis.Arrow.Length));
  end else begin
    ...
    if FAxis.Arrow.Visible then
      p2.X := p2.X + (FDrawer.Scale(FAxis.Arrow.Length));
  end;
...
begin
  if FAxis.IsFlipped then begin
    ...
    if FAxis.Arrow.Visible then
      p1.Y := p1.Y + (FDrawer.Scale(FAxis.Arrow.Length));
  end else begin
    ...
    if FAxis.Arrow.Visible then
      p2.Y := p2.Y - (FDrawer.Scale(FAxis.Arrow.Length));
  end;
...
begin
  ...
  if DistanceToCenter then
    Result := Result div 2;
  Result := Result + (ADrawer.Scale(ATickLength) + ADrawer.Scale(Distance));
end;
