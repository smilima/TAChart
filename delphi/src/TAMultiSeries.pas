...
{$HPPEMIT '#pragma link "TAMultiSeries"'}

interface

uses
  Classes, Graphics, TAChartUtils, TATypes, TACustomSource, TACustomSeries,
  TADrawUtils, TALegend, Types;
...
type
  ...
  TTABubbleSeries = class(TBasicPointSeries)
  private
    FBubbleBrush: TBrush;
    FBubblePen: TPen;
  ...
  {$NODEFINE TBubbleSeries} // C++ must use TTABubbleSeries
  TBubbleSeries = TTABubbleSeries;
...
implementation
uses
  TAFPTypes, Math, SysUtils, TAChartStrConsts, TAGeometry, TAGraph, TAMath;
...
begin
  ...
  center := DivPoint(r.TopLeft + r.BottomRight, 2);
  m := MaxIntValue([FWhiskersWidth, FBoxWidth, 1]) * 2;
  ...
  bw := bw - (IfThen(FBoxPen.Style = psClear, 0, (FBoxPen.Width + 1) div 2));
...
function TTABubbleSeries.AddXY(AX, AY, ARadius: Double; AXLabel: String;
...
procedure TTABubbleSeries.Assign(ASource: TPersistent);
begin
  if ASource is TTABubbleSeries then
    with TTABubbleSeries(ASource) do begin
...
constructor TTABubbleSeries.Create(AOwner: TComponent);
begin
...
destructor TTABubbleSeries.Destroy;
begin
...
procedure TTABubbleSeries.Draw(ADrawer: IChartDrawer);
var
  i: Integer;
  item: PChartDataItem;
  clipR: TRect;
...
function TTABubbleSeries.Extent: TDoubleRect;
...
function TTABubbleSeries.GetBubbleRect(AItem: PChartDataItem; out ARect: TRect): Boolean;
var
...
procedure TTABubbleSeries.GetLegendItems(AItems: TChartLegendItems);
begin
  GetLegendItemsRect(AItems, BubbleBrush);
end;

function TTABubbleSeries.GetNearestPoint(const AParams: TNearestPointParams;
...
function TTABubbleSeries.GetSeriesColor: TColor;
begin
  Result := FBubbleBrush.Color;
end;

procedure TTABubbleSeries.MovePointEx(var AIndex: Integer;
...
procedure TTABubbleSeries.SetBubbleBrush(AValue: TBrush);
begin
...
procedure TTABubbleSeries.SetBubblePen(AValue: TPen);
begin
...
procedure TTABubbleSeries.SetBubbleRadiusUnits(AValue: TBubbleRadiusUnits);
begin
...
procedure TTABubbleSeries.SetOverrideColor(AValue: TBubbleOverrideColors);
begin
...
function TTABubbleSeries.ToolTargetDistance(const AParams: TNearestPointParams;
...
begin
  if Source.YCount < 5 then begin Result := EmptyExtent; exit; end;
...
var
  ...
  tw: Double;
begin
  if Source.YCount < 4 then begin Result := EmptyExtent; exit; end;
...
initialization
  RegisterSeriesClass(TTABubbleSeries, @rsBubbleSeries);
