...
uses
  Classes, TALCLCompat, TAChartUtils, TACustomSource;
...
begin
  ...
  if not IsAnimating then begin Result := Origin.Item[AIndex]; exit; end;
...
begin
  ...
  if FCurrentStep + 1 <> s then
    FSkippedFramesCount := FSkippedFramesCount + 1;
  FCurrentStep := s;
  Notify;
end;
