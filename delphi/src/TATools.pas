...
uses
  Classes, SysUtils, Types, Math, TAFPTypes, // LCL
  Controls, TALCLCompat,
  Forms, // TAChart
...
type
  ...
  TChartToolEvent = procedure (ATool: TTAChartTool; APoint: TPoint) of object;
  ...
  TTAChartTool = class(TBasicChartTool)
  strict private
  ...
  public
    ...
    procedure Dispatch(
      AChart: TChart; AEventId: TChartToolEventId; APoint: TPoint); reintroduce; overload;
  ...
  {$NODEFINE TChartTool} // C++ must use TTAChartTool
  TChartTool = TTAChartTool;
  TChartToolsEnumerator = TTypedFPListEnumerator<TTAChartTool>;
  {$ENDIF}

  TChartToolClass = class of TTAChartTool;
  ...
  strict private
    ...
    FTools: TChartTools;
    function GetItem(AIndex: Integer): TTAChartTool;
  private
    FIsHandled: Boolean;
  protected
  ...
  public
    ...
    property Item[AIndex: Integer]: TTAChartTool read GetItem; default;
  published
    property Tools: TChartTools read FTools;
  end;

  TUserDefinedTool = class(TTAChartTool)
  end;

  { TBasicZoomTool }

  TBasicZoomTool = class(TTAChartTool)
  strict private
  ...
  published
    ...
    property Proportional: Boolean
      read GetProportional write SetProportional stored false default false;  // deprecated
...
type
  ...
  TBasicPanTool = class(TTAChartTool)
  strict private
  ...
  TDataPointTool = class(TTAChartTool)
  public
  type
    TPointRef = class
    private
    ...
    public
      ...
      // Lazarus writes these fields directly from TADataTools, which Delphi
      // does not allow across units, so they are settable through properties.
      procedure SetGraphPos(const ANewPos: TDoublePoint);
      property GraphPos: TDoublePoint read FGraphPos write FGraphPos;
  ...
  TReticuleTool = class(TTAChartTool)
  public
    procedure MouseMove(APoint: TPoint); override;
  end;
...
function InitBuiltinTools(AChart: TTAChart): TBasicChartToolset;
...
uses
  Winapi.Windows, Graphics, TAChartStrConsts, TACustomSeries,
  TAEnumerators, TAGeometry, TAMath;
...
begin
  System.Classes.RegisterClass(AToolClass);
...
procedure TTAChartTool.Activate;
var
  i: Integer;
begin
...
procedure TTAChartTool.AfterDraw(AChart: TChart; ADrawer: IChartDrawer);
begin
...
procedure TTAChartTool.Assign(Source: TPersistent);
begin
  if Source is TTAChartTool then
    with TTAChartTool(Source) do begin
...
procedure TTAChartTool.Cancel;
begin
  // Empty.
end;

constructor TTAChartTool.Create(AOwner: TComponent);
begin
...
procedure TTAChartTool.Deactivate;
begin
  RestoreCursor;
  inherited;
end;

destructor TTAChartTool.Destroy;
begin
  Toolset := nil;
  inherited;
end;

procedure TTAChartTool.Dispatch(
...
procedure TTAChartTool.Draw(AChart: TChart; ADrawer: IChartDrawer);
begin
  Unused(ADrawer);
  FChart := AChart;
end;

function TTAChartTool.EffectiveDrawingMode: TChartToolEffectiveDrawingMode;
begin
  if DrawingMode <> tdmDefault then
    Result := DrawingMode
  else
    // Lazarus picks tdmXor for the gtk and win32 widgetsets; the VCL is
    // always the latter.
    Result := tdmXor;
end;

function TTAChartTool.GetAfterEvent(AIndex: Integer): TChartToolEvent;
begin
  Result := FEventsAfter[TChartToolEventId(AIndex)];
end;

function TTAChartTool.GetBeforeEvent(AIndex: Integer): TChartToolEvent;
begin
  Result := FEventsBefore[TChartToolEventId(AIndex)];
end;

function TTAChartTool.GetIndex: Integer;
begin
  if Toolset = nil then
    Result := -1
  else
...
function TTAChartTool.GetParentComponent: TComponent;
begin
  Result := FToolset;
end;

procedure TTAChartTool.Handled;
begin
  Toolset.FIsHandled := true;
end;

function TTAChartTool.HasParent: Boolean;
begin
  Result := true;
end;

function TTAChartTool.IsActive: Boolean;
begin
  Result := (FChart <> nil) and (FChart.ActiveToolIndex = Index);
end;

procedure TTAChartTool.KeyDown(APoint: TPoint);
begin
  Unused(APoint);
...
procedure TTAChartTool.KeyUp(APoint: TPoint);
begin
  Unused(APoint);
end;

procedure TTAChartTool.MouseDown(APoint: TPoint);
begin
  Unused(APoint);
end;

procedure TTAChartTool.MouseMove(APoint: TPoint);
begin
  Unused(APoint);
end;

procedure TTAChartTool.MouseUp(APoint: TPoint);
begin
  Unused(APoint);
end;

procedure TTAChartTool.MouseWheelDown(APoint: TPoint);
begin
  Unused(APoint);
end;

procedure TTAChartTool.MouseWheelUp(APoint: TPoint);
begin
  Unused(APoint);
end;

function TTAChartTool.PopupMenuConflict: Boolean;
var
  P: TPoint;
begin
  Result := false;
...
procedure TTAChartTool.PrepareDrawingModePen(
...
procedure TTAChartTool.ReadState(Reader: TReader);
begin
...
procedure TTAChartTool.RestoreCursor;
begin
  if ActiveCursor = crDefault then exit;
  FChart.Cursor := FOldCursor;
end;

procedure TTAChartTool.SetActiveCursor(AValue: TCursor);
begin
...
procedure TTAChartTool.SetAfterEvent(AIndex: Integer; AValue: TChartToolEvent);
begin
  FEventsAfter[TChartToolEventId(AIndex)] := AValue;
end;

procedure TTAChartTool.SetBeforeEvent(AIndex: Integer; AValue: TChartToolEvent);
begin
  FEventsBefore[TChartToolEventId(AIndex)] := AValue;
end;

procedure TTAChartTool.SetCursor;
begin
  if ActiveCursor = crDefault then exit;
...
procedure TTAChartTool.SetDrawingMode(AValue: TChartToolDrawingMode);
begin
  if FDrawingMode = AValue then exit;
  FDrawingMode := AValue;
end;

procedure TTAChartTool.SetIndex(AValue: Integer);
begin
  Toolset.Tools.Move(Index, EnsureRange(AValue, 0, Toolset.Tools.Count - 1));
end;

procedure TTAChartTool.SetParentComponent(AParent: TComponent);
begin
  if not (csLoading in ComponentState) then
    Toolset := AParent as TChartToolset;
end;

procedure TTAChartTool.SetToolset(AValue: TChartToolset);
begin
...
procedure TTAChartTool.StartTransparency;
begin
...
var
  candidates: array of TTAChartTool;
...
var
  i, ai: Integer;
begin
  if Tools.Count = 0 then begin Result := false; exit; end;
  ...
  for i := 0 to candidateCount - 1 do begin
    ...
    if FIsHandled then begin Result := true; exit; end;
  end;
  Result := false;
end;
...
function TChartToolset.GetItem(AIndex: Integer): TTAChartTool;
begin
  Result := TTAChartTool(Tools.Items[AIndex]);
end;
...
begin
  if not AdjustSelection or (RatioLimit = zrlNone) then begin Result := FSelectionRect; exit; end;
...
begin
  PanBy(MulPoint(DIR_TO_OFFSET[WheelUpDirection], AStep));
end;
...
procedure TDataPointTool.FindNearestPoint(APoint: TPoint);
  ...
  begin
    ...
    if not RectIntersectsRect(r, ext) then begin Result := false; exit; end;
