...
{$HPPEMIT '#pragma link "TAIntervalSources"'}
...
uses
  DateUtils, Math, StrUtils, SysUtils, TALCLCompat, TAMath;
...
type
  ...
  TDateTimeIntervalsHelper = record
...
begin
  ...
  if step <= 0 then exit;
  SetLength(AValues, Trunc(Min((AParams.FMax - start) / step + 2, MAX_COUNT)));
  for i := 0 to High(AValues) do begin
    // i*step avoids accumulated binary error from repeated m := m + step
    // (which showed up as labels like 0.900000013).
    m := start + i * step;
    if IsZero(m) then
      m := 0;
    AValues[i].FValue := m;
...
var
  ...
  begin
    if DateTimeFormat <> '' then
      begin Result := FormatDateTime(DateTimeFormat, AValue); exit; end;
    DateTimeToChartSystemTime(AValue, st);
...
begin
  ...
  // Delphi requires a simple local variable as the loop control.
  for step in Steps do begin
    helper.FStep := step;
    AParams.FMin := helper.FOrigParams.FMin / DATE_STEP_INTERVALS[step];
    AParams.FMax := helper.FOrigParams.FMax / DATE_STEP_INTERVALS[step];
