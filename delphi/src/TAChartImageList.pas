...
{$HPPEMIT '#pragma link "TAChartImageList"'}

interface

uses
  Classes, Graphics, Controls, TAChartUtils, TACustomSeries, TAGraph, Types;
...
uses
  Math, SysUtils, TADrawUtils, TADrawerCanvas, TAEnumerators, TALegend;
...
begin
  Result := nil;
  AImgIndex := AImgIndex - FFirstSeriesIndex;
  if
...
begin
  Unused(ASender);
  // See TChartListbox.SeriesChanged: the chart broadcasts while it is being
  // destroyed, and rebuilding the icons then would walk a half-freed series
  // list.
