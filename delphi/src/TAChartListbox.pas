...
{$HPPEMIT '#pragma link "TAChartListbox"'}

interface

uses
  Classes, Controls, StdCtrls, TAChartUtils, TACustomSeries, TALegend,
  TAGraph, Types;
...
// Delphi resolves a forward class declaration only within the same type
// section, so the enumerations and SHOW_ALL move above it.
type
  TChartListbox = class;
...
uses
  Winapi.Windows, Graphics, Math, TALCLCompat, SysUtils, Vcl.Themes, TACustomSource,
  TADrawerCanvas, TADrawUtils, TAEnumerators, TAGeometry;
...
begin
  ...
  if cloShowCheckboxes in Options then begin
    ...
    if StyleServices.Enabled then begin
      te := StyleServices.GetElementDetails(THEMED_FLAGS[FCheckStyle, ch]);
      StyleServices.DrawElement(Canvas.Handle, te, rcb);
    end
    else
      DrawFrameControl(
...
begin
  ...
  // LCL: CalculateStandardItemHeight.
  AHeight := Max(Canvas.TextHeight('Wg') + 2, AHeight);
...
begin
  ...
  try
    ...
    p := Point(AX, AY);
    index := ItemAtPos(p, true);   // LCL: GetIndexAtXY
...
begin
  // A chart broadcasts while it is being torn down (TChart.Destroy frees the
  // series list first).  By that point the VCL has already released the
  // listbox's window handle, so repopulating would touch a dead handle and
  // walk a series list that is half destroyed.  The LCL tolerates this; the
  // VCL does not, so the notification is ignored during destruction.
  if (csDestroying in ComponentState) or
     ((FChart <> nil) and (csDestroying in FChart.ComponentState))
  then
    exit;
  if
