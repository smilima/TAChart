...
{$HPPEMIT '#pragma link "TARadialSeries"'}

interface

uses
  Classes, Graphics, SysUtils, Types, TAChartUtils, TACustomSeries,
  TADrawUtils, TALegend;
...
type
  ...
  TTAPolarSeries = class(TBasicPointSeries)
  strict private
    FBrush: TBrush;
  ...
  public
    ...
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function UsesPolarCoordinates: Boolean; override;
  public
    procedure Draw(ADrawer: IChartDrawer); override;
  ...
  {$NODEFINE TPolarSeries} // C++ must use TTAPolarSeries
  TPolarSeries = TTAPolarSeries;

implementation

uses
  Math, TAChartStrConsts, TATypes, TACustomSource, TAGeometry, TAGraph;
...
begin
  for ps in FSlices do begin
    ...
    then
      begin Result := ps.FOrigIndex; exit; end;
  end;
  Result := -1;
end;
  ...
  begin
    with ALabel do begin
      ...
      case MarkPositions of
        pmpAround:
          FCenter := FCenter + (Ofs(AAngle));
        pmpInside:
          FCenter := FCenter - (Ofs(AAngle));
        pmpLeftRight:
          FCenter := FCenter + (Ofs(IfThen(InRange(AAngle, Pi / 2, 3 * Pi / 2), Pi, 0)));
      end;
    ...
    with FSlices[j] do begin
      ...
      if FVisible then begin
        ...
        if Exploded and (di^.X > 0) then
          FBase := FBase + (EndPoint(a, FRadius * di^.X));
...
procedure TTAPolarSeries.Assign(ASource: TPersistent);
begin
  if ASource is TTAPolarSeries then
    with TTAPolarSeries(ASource) do begin
      Self.Brush := FBrush;
...
constructor TTAPolarSeries.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TTAPolarSeries.Destroy;
begin
  FreeAndNil(FBrush);
  FreeAndNil(FLinePen);
  inherited;
...
function TTAPolarSeries.UsesPolarCoordinates: Boolean;
begin
  Result := true;
end;

procedure TTAPolarSeries.Draw(ADrawer: IChartDrawer);
var
  i: Integer;
  cnt: Integer;
  pts: TPointArray;
...
function TTAPolarSeries.Extent: TDoubleRect;
var
  i: Integer;
begin
...
procedure TTAPolarSeries.GetLegendItems(AItems: TChartLegendItems);
var
  p: TSeriesPointer;
begin
  if ShowPoints then
...
function TTAPolarSeries.GetNearestPoint(const AParams: TNearestPointParams;
  out AResults: TNearestPointResults): Boolean;
var
  dist: Integer;
  gp: TDoublePoint;
...
function TTAPolarSeries.GraphPoint(AIndex: Integer): TDoublePoint;
begin
  with Source[AIndex]^, FAngleCache[AIndex] do
    Result := DoublePoint(Y * FCos + OriginX, Y * FSin + OriginY);
end;

function TTAPolarSeries.IsOriginXStored: Boolean;
begin
  Result := OriginX <> 0;
end;

function TTAPolarSeries.IsOriginYStored: Boolean;
begin
  Result := OriginY <> 0;
end;
...
{ ANewPos is in cartesioan coordinates. Convert to polar coordinates and store
 in ListSource }
procedure TTAPolarSeries.MovePoint(var AIndex: Integer;
...
procedure TTAPolarSeries.MovePointEx(var AIndex: Integer;
  AXIndex, AYIndex: Integer; const ANewPos: TDoublePoint);
begin
...
procedure TTAPolarSeries.PrepareAngleCache;
var
  i: Integer;
  s, c: Extended;
begin
...
procedure TTAPolarSeries.PrepareGraphPoints;
var
  s, c: Extended;
  i: Integer;
begin
...
procedure TTAPolarSeries.SetBrush(AValue: TBrush);
begin
...
procedure TTAPolarSeries.SetCloseCircle(AValue: Boolean);
begin
...
procedure TTAPolarSeries.SetFilled(AValue: Boolean);
begin
...
procedure TTAPolarSeries.SetLinePen(AValue: TPen);
begin
...
procedure TTAPolarSeries.SetOriginX(AValue: Double);
begin
...
procedure TTAPolarSeries.SetOriginY(AValue: Double);
begin
...
procedure TTAPolarSeries.SetShowPoints(AValue: Boolean);
begin
...
procedure TTAPolarSeries.SourceChanged(ASender: TObject);
begin
  FAngleCache := nil;
  inherited;
end;
...
initialization
  ...
  RegisterSeriesClass(TTAPolarSeries, @rsPolarSeries);
