...
{$HPPEMIT '#pragma link "TAChartCombos"'}
interface

uses
  Winapi.Windows, Winapi.Messages, SysUtils, Graphics, Classes, Controls,
  StdCtrls,
  TATypes, TAGraph, Types;

type
  TChartComboMode = (ccmPointerStyle, ccmPenStyle, ccmPenWidth, ccmBrushStyle);

  // The LCL additionally offers bsPattern/bsImage brushes and psPattern pens,
  // which the VCL's TBrushStyle and TPenStyle do not have.  The corresponding
  // options (ccoPatternBrush, ccoImageBrush, ccoPatternPen) are therefore not
  // part of the Delphi port.
...
type
  TChartComboBox = class(TCustomComboBox)
    ...
    protected
      ...
      // The LCL overrides RealSetText to keep the selected style when the
      // text is assigned; the VCL's notification for that is CM_TEXTCHANGED.
...
implementation
uses
  TALCLCompat, TypInfo, Math, TAFPTypes, TAChartStrConsts, TAChartUtils,
  TADrawUtils, TADrawerCanvas, TACustomSeries, TASeries, TALegend;
...
begin
  ...
  // LCL sets ReadOnly here; csOwnerDrawFixed is already non-editable
  // in the VCL.
...
begin
  ...
  try
    for ps := Low(TSeriesPointerStyle) to High(TSeriesPointerStyle) do begin
      FBitmaps[ps] := TBitmap.Create;
      FBitmaps[ps].Transparent := true;
      FBitmaps[ps].TransparentColor := TColor(RGB(254, 254, 254));
      FBitmaps[ps].SetSize(AWidth, AHeight);
      FBitmaps[ps].Canvas.Brush.Color := FBitmaps[ps].TransparentColor;
      FBitmaps[ps].Canvas.FillRect(Rect(0, 0, AWidth, AHeight));
...
var
  ps: TSeriesPointerStyle;
begin
  for ps := Low(TSeriesPointerStyle) to High(TSeriesPointerStyle) do
...
begin
  ...
  if Assigned(OnDrawItem) then
      ...
      ccmBrushStyle:
        begin
          ...
          if bs <> bsClear then begin
            ...
            Canvas.FillRect(
              Rect(x1, ARect.Top + MARGIN, x2, ARect.Bottom - MARGIN));
          end;
    ...
    if (ccoNames in FOptions) and (FAlignment <> taCenter) then begin
      ...
      Canvas.Brush.Style := bsClear;
      // LCL: TTextStyle with Layout=tlCenter, EndEllipsis, not opaque.
      Winapi.Windows.DrawText(Canvas.Handle, PChar(txt), Length(txt), ARect,
...
//    Result := TPenStyle(AIndex);
    Result := TPenStyle(NativeInt(Items.Objects[AIndex]));
end;
...
begin
  if AIndex = -1 then
    ...
    Result := TSeriesPointerStyle(NativeInt(Items.Objects[AIndex]));
end;
...
begin
  ...
  try
    Items.Clear;
    for ps := Low(TPenStyle) to High(TPenStyle) do begin
      Items.AddObject(GetPenStyleName(ps), TObject(NativeInt(ps)));
    end;
  finally
    Items.EndUpdate;
    dec(FLockItemIndex);
...
begin
  ...
  try
    for i:=0 to High(LIST) do begin
      ...
      s := GetSymbolName(ps);
      if s <> '' then
        styleItems.AddObject(s, TObject(NativeInt(ps)));
    end;
...
procedure TChartComboBox.CMTextChanged(var AMessage: TMessage);
begin
  inherited;
  // FBrushStyle and friends always mirror the current selection (see
  // SetItemIndex), so they carry the style that has to survive the change.
  // Re-selecting moves ItemIndex, which changes the text again, hence the lock.
  if FLockItemIndex <> 0 then exit;
  Inc(FLockItemIndex);
  try
    case FMode of
      ccmBrushStyle   : SetSelectedBrushStyle(FBrushStyle);
      ccmPenStyle     : SetSelectedPenStyle(FPenStyle);
      ccmPenWidth     : SetSelectedPenWidth(FPenWidth);
      ccmPointerStyle : SetSelectedPointerStyle(FPointerStyle);
    end;
  finally
    Dec(FLockItemIndex);
  end;
